...
void KrakenIO::SendSensorsToDash() {
    ...
    Dashboard::LogFalcon(driveMotorName, _canDriveMotor);
    Dashboard::LogFalcon(turnMotorName, _canTurnMotor);
}
...
void KrakenIO::SetDesiredVelocity(units::meters_per_second_t velocity, units::newton_t forceFF) {
    ...
    torqueVoltageFF *= Dashboard::Tune("swerve/volatgeFF enabled", true);
    ...
    Dashboard::Log("swerve/drive " + std::to_string(_canDriveMotor.GetDeviceID()) + " torqueVoltage", torqueVoltageFF);
}
